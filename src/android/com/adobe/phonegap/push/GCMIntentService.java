...
public class GCMIntentService extends GcmListenerService implements PushConstants {
    ...
    private String normalizeKey(String key) {
        if (key.equals(APS_ALERT) || key.equals(BODY) || key.equals(ALERT) || key.equals(GCM_NOTIFICATION_BODY) || key.equals(TWILIO_BODY)) {
            return MESSAGE;
        } else if (key.equals(TWILIO_TITLE)) {
            return TITLE;
        }else if (key.equals(APS_BADGE) || key.equals(MSGCNT) || key.equals(BADGE)) {
            return COUNT;
        } else if (key.equals(APS_SOUND) || key.equals(SOUNDNAME) || key.equals(TWILIO_SOUND)) {
            return SOUND;
    ...
    public void createNotification(Context context, Bundle extras) {
        ...
        String packageName = context.getPackageName();
        Resources resources = context.getResources();
        Spanned title = fromHtml(extras.getString(TITLE));

        if (title == null)
            title = fromHtml(appName);
